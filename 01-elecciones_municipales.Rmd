...
writexl::write_xlsx(df_completa, "output/elecciones municipales 1992 2021.xlsx")
...
df_n_candidatos <- df_completa_digitacion %>% 
  group_by(fecha, comuna, coalicion) %>% 
...
df_completa <- left_join(df_completa, df_n_candidatos)

rm(df_listas, df_completa_digitacion, df_completa_votacion, df_n_candidatos)
```
...
df_electoral_municipal %>% 
  filter(comuna == "rio claro") %>% 
  select(fecha, comuna, periodo_electoral, participacion_alcalde_incum, elecciones_no_competitivas)

# df_consolidada <- df_electoral_municipal %>% 
#   select(comuna, cod_com, candidato_pm, periodo_electoral, fecha, participacion_alcalde_incum, elecciones_no_competitivas, votos_candidato_pm_p)
# 
# writexl::write_xlsx(df_consolidada, "output/231112 - Participación.xlsx")
```

```{r}

df_participacion <- readxl::read_excel("input/231115 - Participación_rec.xlsx") %>%
...
colnames(df_completa_incumbente)

df_completa_incumbente <- df_completa_incumbente %>% left_join(df_participacion)
...
```{r}
df_incumbente <- df_completa_incumbente %>% 
  select(fecha, comuna, candidato_pm, coalicion_pm, reeleccion_alcalde, participacion_alcalde_incum, votos_validos) %>% 
...
df_completa_alcaldes <- df_completa_alcaldes %>% 
  mutate(across(where(is.character), 
                \(x) stringi::stri_trans_general(str_to_lower(x), "latin-ascii"))) %>% 
  select(fecha, comuna, candidato, sexo, pacto, sigla_partido, votos_candidato, partido, pacto_nombre, coalicion)
  
df_experimento <- left_join(df_incumbente, df_completa_alcaldes, by = join_by(fecha, comuna, incumbente == candidato))
...
```{r}
df_n_periodos <- df_completa_incumbente %>% 
  group_by(comuna) %>% 
  select(fecha, comuna, candidato_pm, coalicion_pm, reeleccion_alcalde, participacion_alcalde_incum, periodos_consecutivos_alcalde, periodos_consecutivos_coalicion) %>% 
...
- Número de periodos coalicion y alcalde: df_n_periodos
- % coalicion incumbente: df_coalicion_p
...
df_coalicion_p <- df_completa_incumbente %>% 
  select(fecha, comuna, candidato_pm, votos_candidato_pm, coalicion_pm, votos_coalicion, votos_candidato_pm_p, 
         votos_coalicion, coalicion_municipal_incumbente, votos_validos, otros_n_candidatos:frente_amplio_n_candidatos, 
         coalicion_concertacion_p, coalicion_alianza_p, coalicion_independientes_p, coalicion_otros_p, 
         coalicion_frente_amplio_p) %>% 
  rowwise() %>% 
  ...
  select(fecha, comuna, candidato_pm, coalicion_pm, coalicion_municipal_incumbente, votos_candidato_pm_p, votos_coalicion_p, 
         votos_coalicion_incumbente_p,concertacion_n_candidatos, alianza_n_candidatos, otros_n_candidatos,
         independientes_n_candidatos) 
...
df_final <- df_completa_incumbente %>%
...
write_dta(df_final, "data/231115 - Elecciones municipales.dta")
write_rds(df_final, "data/231115 - Elecciones municipales.rds")
write_sav(df_final, "data/231115 - Elecciones municipales.sav")
