...
df_incumbent <- df_incumbent_participation %>% 
  select(fecha, comuna, candidato_pm, sexo_pm, sigla_partido_pm, pacto_nombre_pm, pacto_pm, partido_pm, 
...
df_all_candidates_by_year <- df_all_candidates_by_year %>% 
  filter(fecha > 2000) %>% 
  select(-tipo_eleccion, -cargo, sexo_incumbente = sexo, pacto_incumbente = pacto,
         sigla_partido_incumbente = sigla_partido, votos_incumbente = votos_candidato, 
...
#df_gender <- readxl::read_xlsx("input/231030 - Género_rec.xlsx")
...
df_elect_and_incumbent <- df_join %>% 
  mutate(coalicion_incumbente = if_else(reeleccion_alcalde == 1, coalicion_pm, coalicion_incumbente),
         ...
         sigla_partido_incumbente = if_else(reeleccion_alcalde == 1, sigla_partido_pm, sigla_partido_incumbente),
...
df_elect_and_incumbent <- readxl::read_xlsx("input/electo e incumbente.xlsx")

colnames(df_elect_and_incumbent)

df_incumbent_participation <- df_incumbent_participation %>% 
  left_join(df_elect_and_incumbent)

# col_comunas <- df_elect_and_incumbent %>% 
#   filter(participacion_alcalde_incum == 1 & reeleccion_alcalde == 0 & is.na(pacto_incumbente)) %>% 
#   select(comuna) %>% 
#   distinct() %>% 
#   pull()
# 
# df_all_candidates_by_year %>% 
#   filter(comuna %in% col_comunas)  %>%
#   writexl::write_xlsx("output/incumbente datos faltantes.xlsx")
